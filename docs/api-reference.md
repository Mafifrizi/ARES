...
API keys are generated inside ARES after a user has authenticated. They are for
automation and integrations that call API endpoints with `X-API-Key`. They are
separate from deployment secrets such as `ARES_SECRET_KEY` and
`ARES_ENCRYPTION_KEY`, which must be generated by the deployer before startup.
