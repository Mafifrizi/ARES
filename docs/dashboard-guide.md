...
| New modules or UI updates released | Operator needs new capabilities without re-cloning repo. | Run `ares update` to add new modules, or `ares upgrade` to upgrade the full system (core engine, database schemas, modules, Web UI) with zero user data loss. |
