...
| MOD-016 | ad.sccm cleartext credential evaporation & DPAPI misleading | FIXED | PASSED (42/42 pipeline tests) | `ares/normalize/artifacts.py`, `tests/unit/test_artifact_normalizer_pipeline.py` | Handler `_normalize_cleartext_credentials` di `ArtifactNormalizer` Grup A (commit `9f8b111`). |
...
| MOD-021 | lateral.rdp false-positive execution claim on open port | FIXED | PASSED (Unit tests & execution flow) | `ares/modules/lateral/modules.py` | Kalibrasi status: port 3389 terbuka dilaporkan sebagai info/port discovery (`rdp_port_open`), tidak mengklaim lateral execution success tanpa NLA auth. |
| MOD-022 | exfil.staged_collection workstation egress disruption & false target attribution | FIXED | PASSED (Unit tests & before_request integration) | `ares/modules/exfil/staged_collection.py` | Penegakan `before_request(target, "ssh")` di awal eksekusi mencegah probing di luar scope campaign. |
| MOD-023 | exfil.staged_collection phantom staging pipeline & 100% data loss | FIXED | PASSED (Unit tests & output mapping) | `ares/modules/exfil/staged_collection.py` | Output `sensitive_file_paths` dan `collection_inventory` dipetakan ke hasil pencarian file. |
| MOD-024 | network.port_scan CIDR parsing incompatibility & socket failure | FIXED | PASSED (Unit tests & validation) | `ares/modules/network/port_scan.py` | Validasi target single IP/hostname via `before_request(target, "tcp_scan")` dan penanganan error terstruktur. |
...
| MOD-036 | credential.ticket_converter output key mismatch & pipeline evaporation | FIXED | PASSED (42/42 pipeline tests) | `ares/normalize/artifacts.py`, `tests/unit/test_artifact_normalizer_pipeline.py` | Handler `_normalize_converted_ticket` di `ArtifactNormalizer` Grup A (commit `9f8b111`). |
...
| MOD-053 | cloud.aws_privesc & cloud.identity_federation_abuse unvalidated AWS account/tenant scope & generic host attribution | FIXED | PASSED (12/12 CloudScopeGuard tests) | `ares/core/scope.py`, `ares/core/campaign.py`, `ares/modules/cloud/*`, `tests/unit/test_cloud_scope_guard.py` | Validasi cloud identifier (aws_account_ids, tenant_ids) via CloudScopeGuard di method run() (commit `2e6d71b`). |
...
| MOD-056 | ad.enum_users unhandled output telemetry password_policy & dual-write omission | FIXED | PASSED (Unit tests & normalizer pipeline) | `ares/modules/ad/enum_users.py`, `ares/normalize/artifacts.py`, `tests/unit/test_artifact_normalizer_pipeline.py` | Dual-write `users` key ke `raw` dan handler `_normalize_password_policy` di `ArtifactNormalizer` Grup A (commit `9f8b111`). |
...
| MOD-063 | cloud.* unscoped cloud account/tenant execution via ambient credentials | FIXED | PASSED (12/12 CloudScopeGuard tests) | `ares/core/scope.py`, `ares/core/campaign.py`, `ares/modules/cloud/*`, `tests/unit/test_cloud_scope_guard.py` | Validasi cloud identifier (aws_account_ids, azure_subscription_ids, azure_tenant_ids, gcp_project_ids) via CloudScopeGuard (commit `2e6d71b`). |
...
## CRITICAL FINDINGS (RESOLVED IN FASE 4 — GUARANTEED TEARDOWN)

### [MOD-012] Persistent Backdoor Machine Account & DACL Injection on `lateral.ntlm_relay`
- **Severity**: **CRITICAL**
- **Status**: **FIXED** (commit `28e983a` — Fase 4 Grup C)
- **Ringkasan Teknis**:
  Pada method `_rbcd_attack()` (`ares/modules/lateral/ntlm_relay.py`), initial DACL kini disimpan sebelum modifikasi. Akun mesin `ARESXXXXXX$` dihapus via `conn.delete()` dan initial DACL dipulihkan di blok `finally`. 2/2 unit tests passing di `test_ntlm_relay_teardown.py`.

### [MOD-046] Zero Teardown on Remote Scheduled Task & Registry Run Key Persistence on `persistence.scheduled_task`
- **Severity**: **CRITICAL**
- **Status**: **FIXED** (commit `327458b` — Fase 4 Grup C)
- **Ringkasan Teknis**:
  `created_artifacts` disimpan sebelum pembuatan task; penghapusan scheduled task via RPC di blok `finally` (termasuk kegagalan midway) serta method `teardown()` eksplisit. Autorun registry run key dibersihkan via RPC `hBaseRegDeleteValue`. 3/3 tests passing di `test_scheduled_task_teardown.py`.
...
- **Severity**: **HIGH**
- **Status**: **FIXED** (commit `2e6d71b` - Fase 6 CloudScopeGuard)
- **Catatan**: CloudScopeGuard mengintegrasikan validasi cloud identifier (`aws_account_ids`, `azure_subscription_ids`, `azure_tenant_ids`, `gcp_project_ids`) pada `CampaignScope`. Seluruh modul cloud (`cloud.aws`, `cloud.azure`, `cloud.azure_ad`, `cloud.gcp`, `cloud.aws_privesc`) memanggil `validate_cloud_scope()` setelah identifier di-resolve. Empty list = unrestricted (backward compatible).
