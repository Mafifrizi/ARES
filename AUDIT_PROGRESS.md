...
| MOD-018 | network.pivot teardown omission on background SSH processes | TERBUKTI (HIGH) | Audit verified (`pivot.py:343-359`) | Menunggu konfirmasi arsitektur | `PivotModule.teardown()` tidak pernah dipanggil di engine lifecycle, tidak ada mekanisme teardown lifecycle modul di engine. STOP menunggu konfirmasi (Rule 4). |
